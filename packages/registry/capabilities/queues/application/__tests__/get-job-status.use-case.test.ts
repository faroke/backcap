...
describe("GetJobStatus use case", () => {
  ...
  beforeEach(async () => {
    ...
    await jobRepo.save(createTestJob({ id: "job-1", queue: "emails" }));
  });

  it("returns job status", async () => {
    const result = await getJobStatus.execute("job-1");

    expect(result.isOk()).toBe(true);
    const output = result.unwrap();
    expect(output.id).toBe("job-1");
    expect(output.queue).toBe("emails");
    expect(output.status).toBe("pending");
  });

  it("fails when job not found", async () => {
    const result = await getJobStatus.execute("nonexistent");
