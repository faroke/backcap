...
describe("init tsconfig prerequisite", () => {
  ...
  it("handles tsconfig.json with comments and trailing commas (tsc --init)", async () => {
    // Simulates the tsconfig.json generated by `npx tsc --init`
    const tscInitTsconfig = `{
  "compilerOptions": {
    /* Visit https://aka.ms/tsconfig to read more */
    "target": "es2016",                                  /* Set the JavaScript language version */
    // "lib": [],                                        /* Specify a set of library files */
    "module": "commonjs",                                /* Specify what module code is generated */
    // "rootDir": "./",                                  /* Specify the root folder */
    "strict": true,                                      /* Enable all strict type-checking options */
    "esModuleInterop": true,                             /* Emit additional JS to ease interop */
    ...
    mockReadFile.mockImplementation((path) => {
      if (String(path).includes("tsconfig.json")) {
        return Promise.resolve(tscInitTsconfig);
      }
      return Promise.reject(new Error("ENOENT"));
    });
    ...
    const writtenTsconfig = JSON.parse(tsconfigWriteCall![1] as string);
    expect(writtenTsconfig.compilerOptions.paths["@domains/*"]).toEqual(["domains/*"]);
    expect(writtenTsconfig.compilerOptions.target).toBe("es2016");
    expect(writtenTsconfig.compilerOptions.strict).toBe(true);
    // Should not have fail() called
